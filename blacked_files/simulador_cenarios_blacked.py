...
from gerador_rapido_de_cenario import simulacao
...
class EstrategiaAdaptada(Strategy):
    ...
    def init(self):
        ...
        self.datas_validas = (
            self._gerar_datas_validas()
        )  # Lista de datas válidas para negociação
    ...
    def _gerar_datas_validas(self):
        ...
        self.intervalos_valendo["data_ini_periodo"] = pd.to_datetime(
            self.intervalos_valendo["data_ini_periodo"]
        )
        self.intervalos_valendo["data_fim_periodo"] = pd.to_datetime(
            self.intervalos_valendo["data_fim_periodo"]
        )
    ...
    def next(self):
        ...
        if self.pos_ant == 0:  # Sem posição
            if evento == 1:  # Sinal de compra
                ...
                self.ini_posicao_ant = (
                    ind  # Marca a barra em que a posição foi comprada
                )
                self.valor_posicao_ant = valor
                # Assume que posição adquirida foi o preço de fechamento
                # da barra sendo inspecionada
                ...
                self.ini_posicao_ant = ind  # Marca a barra em que a posição foi vendida
                  # Assume que posição vendida foi o preço de
                  # fechamento da barra sendo inspecionada
                self.valor_posicao_ant = valor

        # Lógica de decisão caso a posição anterior fosse vendida
        elif self.pos_ant == -1:  # Posição vendida
            if (
                -(valor / self.valor_posicao_ant - 1) > self.pt
                or -(valor / self.valor_posicao_ant - 1) < -self.sl
                or evento == 1
            ):  # Testa stop loss ou profit taking ou sinal de compra
        ...
        # Lógica de decisão caso a posição anterior fosse comprada
            ...
            if (
                (valor - self.valor_posicao_ant) / self.valor_posicao_ant > self.pt
                or (valor - self.valor_posicao_ant) / self.valor_posicao_ant < -self.sl
                or evento == 2
            ):
                self.position.close()  # Fecha a posição
                self.pos_ant = 0  # Muda variável de estado para neutro
...
def processa_simulacao(
    ...
    file_name_trades = (
        f"{folder_path}/{ticker_clean}_{file_name_tipo_intervalo}_trades.csv"
    )
    file_name_equity_curve = (
        f"{folder_path}/{ticker_clean}_{file_name_tipo_intervalo}_equity_curve.csv"
    )
    ...
    # A seguir há dois caminhos de importar os dados
    # importante escolher o deseja comentando o caminho indesejado
    ...
    # Ou importa os dados da base

    if ticker:

        file_name = (
            f"base_dados_ativos_simulados/{ticker_clean}/{tipo_intervalo}_dados.csv"
        )
    ...
    # Converte as colunas para tipo numérico (ignorar erros caso já esteja no formato correto)
    data[colunas_numericas] = data[colunas_numericas].apply(
        ...
        for janela_rsi in janela_rsi_vals:
            for ordem in ordem_vals:
                for lookback in lookback_vals:
                    print(
                        f"Simulando para: Ativo={ticker_clean}, janela_rsi={janela_rsi}, ordem={ordem}, lookback = {lookback}, d_max={d_max}, "
                        f"break_min={break_min}, aplicar_log={aplicar_log}"
                    )

                    # Chama a função de simulação para gerar os sinais
                    breaks_gerados = simulacao(
                        janela_rsi,
                        ordem,
                        lookback,
                        d_max,
                    ...
                    data["Evento"] = (
                        0  # Zera os valor de Evento em data para colocar os Eventos de break
                    )

                    for _, row in breaks_gerados.iterrows():
                        x_rompimento = int(
                            row["x_rompimento"]
                        )  # Certifique-se de que ponto é um inteiro
                        evento = row["evento"]
                        if (
                            0 <= x_rompimento - 1 < len(data)
                        ):  # Verifique se está dentro dos limites
                            data.iloc[
                                x_rompimento - 1, data.columns.get_loc("Evento")
                            ] = evento
                    ...
                    dados_backtesting = pd.DataFrame(
                        ...
                        index=data.index,
                    )  # Define o índice como a coluna 'Date'
                    ...
                    for sl_val in sl_vals:
                        for pt_val in pt_vals:
                            ...
                            resultados = pd.concat(
                                [resultados, pd.DataFrame([nova_linha])],
                                ignore_index=True,
                            )
                            ...
                            if imprime_grafico:
                                ...
                                bt.plot(
                                    open_browser=True, filename=file_name_backtesting
                                )
...
# ---------------------------------------------------------------------
# Executando propriamente a simulação desejada
# ---------------------------------------------------------------------
...
num_pontos = 2  # pontos mínimos que uma reta tem que passar para ser relevante
...
imprime_grafico = (
    False  # flag se gera ou não os gráficos, só é usado no cenário escolhido (único)
)
salva_dados = (
    False  # flag se gera ou não arquivos, só é usado no cenário escolhido (único)
)
...
for ativo in ativo_vals:  # aqui começa o loop ativo a ativo
    ...
    # Criação do DataFrame para armazenar os resultados do
    # cenario_in_sample_1x do ativo sendo simulado
    resultados_cenario_in_sample_1x = pd.DataFrame(
        columns=[
            "janela_rsi",
            "ordem",
            "lookback",
    ...
    for (
        ...
        cenarios_simulados
    ):  # Roda os cenários previstos em cenario_in_sample_1x no caso, 1, 2 e 3

        match cenario:

            case (
                1
            ):  # ------------ Bloco in_sample_1_1x ----------------------------------

                start_date = "2000-01-01"
                end_date = "2005-08-19"
                break_min_vals = [3, 4]
                lookback_vals = [50, 100]
                janela_rsi_vals = [28, 35, 42, 49]
                ordem_vals = [2, 3, 4]
                sl_vals = [0.02]
                pt_vals = [0.05]
        ...
        resultado_simulado = processa_simulacao(
            ticker,  # executa a simulação e guarda no dataframe
            tipo_intervalo,
            start_date,
            end_date,
            break_min_vals,
    ...
    resultados_cenario_in_sample_1x["trades"] = pd.to_numeric(
        resultados_cenario_in_sample_1x["trades"], errors="coerce"
    ).astype(float)

    # Salva o arquivo com os resultados de resultados_cenario_in_sample_1x

    folder_path = f"simulações_aplicadas_a_ativos/{ticker_clean}"
    file_name_resultado_cenário = (
        f"{folder_path}/{ticker_clean}_cenario_in_sample_1x_resultados.csv"
    )
    resultados_cenario_in_sample_1x.to_csv(
        file_name_resultado_cenário, sep=";", decimal=",", index=True, encoding="utf-8"
    )

    # -----------------------------------------------------------------
    # Após se acumular no dataframe resultados_cenario_in_sample_1x
    # todos os cenários previstos, vamos obter a média dos itens
    # 'return_ann', 'sharpe_ratio', 'max_drawdown', 'trades'
    # -----------------------------------------------------------------
    ...
    # Com isso, o dataframe resultado_media_in_sample_1x tem,
    # para os diferentes cenários, as médias obtidas
    
    resultado_media_in_sample_1x = resultados_cenario_in_sample_1x.groupby(
        colunas_agrupamento, as_index=False
    )[colunas_media].mean()

    # Agora criamos as notas obtidas entre 0 e 10, conforme o valor pro-rata
    # entre a menor média e a maior média

    resultado_media_in_sample_1x["Pontos_return_ann"] = (
        10
        * (
            resultado_media_in_sample_1x["return_ann"]
            - resultado_media_in_sample_1x["return_ann"].min()
        )
        / (
            resultado_media_in_sample_1x["return_ann"].max()
            - resultado_media_in_sample_1x["return_ann"].min()
        )
    )
    ...
    resultado_media_in_sample_1x["Pontos_sharpe_ratio"] = (
        10
        * (
            resultado_media_in_sample_1x["sharpe_ratio"]
            - resultado_media_in_sample_1x["sharpe_ratio"].min()
        )
        / (
            resultado_media_in_sample_1x["sharpe_ratio"].max()
            - resultado_media_in_sample_1x["sharpe_ratio"].min()
        )
    )
    ...
    resultado_media_in_sample_1x["Pontos_max_drawdown"] = (
        10
        * (
            resultado_media_in_sample_1x["max_drawdown"]
            - resultado_media_in_sample_1x["max_drawdown"].min()
        )
        / (
            resultado_media_in_sample_1x["max_drawdown"].max()
            - resultado_media_in_sample_1x["max_drawdown"].min()
        )
    )
    ...
    resultado_media_in_sample_1x["Pontos_trades"] = (
        10
        * (
            resultado_media_in_sample_1x["trades"].astype(float)
            - resultado_media_in_sample_1x["trades"].astype(float).min()
        )
        / (
            resultado_media_in_sample_1x["trades"].astype(float).max()
            - resultado_media_in_sample_1x["trades"].astype(float).min()
        )
    )
    ...
    resultado_media_in_sample_1x = resultado_media_in_sample_1x.sort_values(
        by=colunas_ordenacao
    )

    # Salvando o DataFrame em um arquivo CSV
    file_name_resultado_media_in_sample_1x = (
        f"{folder_path}/{ticker_clean}_cenario_in_sample_1x_medias.csv"
    )
    resultado_media_in_sample_1x.to_csv(
        file_name_resultado_media_in_sample_1x,
        sep=";",
        decimal=",",
        index=True,
    ...
    # Criação do DataFrame para armazenar os melhores resultados
    # do cenario_in_sample_1x do ativo sendo simulado
    melhores_resultados_cenario_in_sample_1x = pd.DataFrame(
        columns=[
            "ativo",
            "cenario",
            "variavel",
    ...
    df_melhor_janela_rsi_in_sample_1 = resultado_media_in_sample_1x.groupby(
        "janela_rsi"
    )["Pontos_ponderados"].mean()
    ...
    df_melhor_ordem_in_sample_1 = resultado_media_in_sample_1x.groupby("ordem")[
        "Pontos_ponderados"
    ].mean()
    ...
    df_melhor_lookback_in_sample_1 = resultado_media_in_sample_1x.groupby("lookback")[
        "Pontos_ponderados"
    ].mean()
    ...
    df_melhor_break_min_in_sample_1 = resultado_media_in_sample_1x.groupby("break_min")[
        "Pontos_ponderados"
    ].mean()
    ...
    file_name_resultado_media_in_sample_1x = (
        f"{folder_path}/{ticker_clean}_cenario_in_sample_1x_melhores_resultados.csv"
    )
    melhores_resultados_cenario_in_sample_1x.to_csv(
        file_name_resultado_media_in_sample_1x,
        sep=";",
        decimal=",",
        index=True,
    ...
    folder_path_melhores_resultados = f'simulações_aplicadas_a_ativos_in_sample_1'
    ...
    file_name_resultados = "simulações_aplicadas_a_ativos/resultados_finais_acumulados_por_ativo_intervalo.csv"
    ...
    file_name_resultados = "simulações_aplicadas_a_ativos/resultados_finais_acumulados_por_ativo.csv"
    resultados_existentes = pd.read_csv(file_name_resultados, sep=";", decimal=",", index_col=0, encoding="utf-8")
