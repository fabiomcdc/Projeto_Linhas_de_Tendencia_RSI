...
# -------------------------------------------------------------------------
# Rotina que checa a posição relativa de um ponto acima ou abaixo
# de uma linha de tendência
# -------------------------------------------------------------------------

def checar_acima_abaixo_da_linha(slope, pivot_x, pivot_y, df, acima: bool):

    # Verifica se algum ponto de df está acima ou abaixo
    # da linha definida por (pivot_x, pivot_y) e slope.
    ...
    for index, row in df.iterrows():
        ...
        if int(x) != int(pivot_x):  # Evito comparaR com o próprio pivot
            if acima and y_on_line - math.trunc(y * 10000) / 10000 > 0.01:
                # Se for linha de suporte e y estiver abaixo da linha, retorne False
                return False
            elif not acima and y - math.trunc(y_on_line * 10000) / 10000 > 0.01:
                # Se for linha de resistência e y estiver acima da linha, retorne False
                return False

    # Se nenhum ponto estiver acima ou abaixo da linha conforme necessário,
    # retornA True
    return True


# -------------------------------------------------------------------------
# Rotinas que criam linhas de tendência dentro de uma janela
# -------------------------------------------------------------------------

def rotacionar_ate_tocar(
    tops_bottoms_df, pivot_x, pivot_y, slope, direction, support: bool
):

    inclinacoes = []

    # Percorre todos os pontos do DataFrame e calcular as inclinações
    for index, row in tops_bottoms_df.iterrows():
        x = row["top_bottom_idx"]
        y = row["top_bottom_value"]

        # Calcula a inclinação da reta ligando o ponto pivot ao ponto atual
        # Verifica se o divisor não é zero antes de calcular a inclinação
        if x != pivot_x:
        ...
        if support:
            ...
            # as retas entre o pivot e  os outros mínimos que possuam
            # inclinação MENOR (ou MAIS NEGATIVA) do que a inclinação da reta da regressão.
            # Se o sentido for anti-horário (direction = 1), preciso considerar
            # as retas entre o pivot e os outros mínimos que possuam 
            # inclinação MAIOR (ou MENOS NEGATIVA) do que a inclinação da reta da regressão.
            if direction == 1:
                if inclinacao < slope:
    ...
    if direction == 1:
        ...
        # Se estiver procurando no sentido horário (direction = 1), 
        # preciso considerar primeiro as retas de MENOR inclinação
    ...
    if support:
        for inclinacao in inclinacoes:
            if checar_acima_abaixo_da_linha(
                inclinacao, pivot_x, pivot_y, tops_bottoms_df, True
            ):
                return inclinacao
    else:
        # Se estiver fazendo as retas de resistência (superiores, support = False)...
        ...
        for inclinacao in inclinacoes:
            if checar_acima_abaixo_da_linha(
                inclinacao, pivot_x, pivot_y, tops_bottoms_df, False
            ):
                return inclinacao
...
def ajustar_linha_de_tendencia(
    data: np.array, borda_esquerda: int, tops_bottoms_df: pd.DataFrame, support: bool
):
    ...
    x_full = np.arange(
        borda_esquerda, borda_esquerda + len(data)
    )  # Índices para todos os dados
    coefs = np.polyfit(x_full, data, 1)

    # Pontos da linha de tendência (y) correspondentes aos
    # valores de top_bottom_idx em tops_df e bottoms_df
    line_points_tops_bottoms = coefs[0] * x_tops_bottoms + coefs[1]

    # Calcula as diferenças entre line_points e top_bottom_value para os máximos

    tops_bottoms_df = tops_bottoms_df.copy()

    if support:
        tops_bottoms_df.loc[:, "diferenca"] = (
            line_points_tops_bottoms - tops_bottoms_df["top_bottom_value"]
        )
    else:
        tops_bottoms_df.loc[:, "diferenca"] = (
            tops_bottoms_df["top_bottom_value"] - line_points_tops_bottoms
        )

    # Acha o máximo mais distante verticalmente da reta
    pivot = tops_bottoms_df.loc[tops_bottoms_df["diferenca"].idxmax()]
    pivot_y = tops_bottoms_df.loc[
        tops_bottoms_df["top_bottom_idx"] == pivot["top_bottom_idx"], "top_bottom_value"
    ].values[0]

    # Agora, otimiza a inclinação para as linhas de tendência usando
    # a rotação sentido horário e depois sentido anti horário

    slope_1 = rotacionar_ate_tocar(
        tops_bottoms_df, pivot["top_bottom_idx"], pivot_y, coefs[0], -1, support
    )

    slope_2 = rotacionar_ate_tocar(
        tops_bottoms_df, pivot["top_bottom_idx"], pivot_y, coefs[0], 1, support
    )
...
# -------------------------------------------------------------------------
# Rotinas que mapeam por quantos pontos uma linha de tendência passa
# -------------------------------------------------------------------------

def mapear_retas_com_bottoms(bottoms, retas, dist_min, num_pontos):
    mapeamento_suporte = pd.DataFrame(
        index=bottoms["top_bottom_idx"], columns=retas["indice"]
    )

    # Preenche o DataFrame mapeamento
    for bottom in bottoms.itertuples():
        for reta in retas.itertuples():
            valor_na_reta = (
                bottom.top_bottom_idx * reta.support_slope + reta.support_intercept
            )
            ...
            if mapeamento_suporte.index[i] <= fim_janela:
                if valor == 0:
                    ...
                    if zeros_consecutivos >= num_pontos:  # sequencia bem sucedida
                        sucesso = True
                        if (
                            zeros_consecutivos > maior_zeros
                        ):  # nova sequencia tem mais zeros
...
def mapear_retas_com_tops(tops, retas, dist_min, num_pontos):
    mapeamento_resistencia = pd.DataFrame(
        index=tops["top_bottom_idx"], columns=retas["indice"]
    )
    ...
    for top in tops.itertuples():
        for reta in retas.itertuples():
            valor_na_reta = (
                top.top_bottom_idx * reta.resist_slope + reta.resist_intercept
            )
...
# -------------------------------------------------------------------------
# Rotinas que eliminam linhas de tendência iguais
# -------------------------------------------------------------------------
...
def identificar_retas_similares_suporte(df):
    ...
    grouped_df = (
        df.groupby(
            ["indice_original_lower_pivot", "support_slope", "support_intercept"]
        )
        .agg(
            {
...
def identificar_retas_similares_resistencia(df):
    ...
    grouped_df = (
        df.groupby(["indice_original_upper_pivot", "resist_slope", "resist_intercept"])
        .agg(
            {
                "valor_rsi": "first",  # Selecionar o primeiro valor de valor_rsi
                "mapeado": lambda x: 1,  # Atribuir 1 para todos os grupos
...
def checar_cruzou_para_baixo(
    ind_pontos, slope_reta_suporte, intercept_reta_suporte, break_min, dados_rsi
):
    if len(ind_pontos) != 2:
    ...
    if (
        primeiro_acima and segundo_abaixo_distante
    ):  
        # Ocorreu um cruzamento de x1 para x2, agora temos que achar primeiro
        # ponto em que esse cruzamento ocorreu (x3)
        # Procura por x3 entre x1 e x2, começando do primeiro x maior do que x1, indo até x2

        for x in range(int(x1) + 1, int(x2)):
            y3 = dados_rsi.iloc[x][
                "RSI"
            ]  # Obtendo o valor de RSI correspondente ao x3 sendo testado => y3
            y_reta_3 = (
                slope_reta_suporte * x + intercept_reta_suporte
            )  # Obtendo o valor na reta correspondente ao x3 sendo testado => y_reta_3
            if (
                y3 < y_reta_3 and abs(y3 - y_reta_3) > break_min
            ): 
                # Se y3 < y_reta_3, é sinal que houve rompimento no ponto x3 sendo testado,
                # interrompo o teste e retorno x e y3, caso contrário, continuo
        ...
        # Retorno criado para evitar mensagem de erro pois loop
        # for deveria encontrar x3 válido, nem que seja igual a x2
    else:
...
def checar_cruzou_para_cima(
    ind_pontos, slope_reta_resistencia, intercept_reta_resistencia, break_min, dados_rsi
):
    if len(ind_pontos) != 2:
        raise ValueError("A lista ind_pontos deve conter exatamente dois pontos.")
    ...
    if primeiro_abaixo and segundo_acima_distante:
        ...
        for x in range(int(x1) + 1, int(x2)):
            ...
            y_reta_3 = (
                slope_reta_resistencia * x + intercept_reta_resistencia
            )  # Obtendo o valor na reta correspondente ao x3 sendo testado => y_reta_3
            if (
                y3 > y_reta_3 and abs(y3 - y_reta_3) > break_min
            ):
        ...
        # Retorno criado para evitar mensagem de erro pois loop for
        # deveria encontrar x3 válido, nem que seja igual a x2
    else:
        return False, None, None
